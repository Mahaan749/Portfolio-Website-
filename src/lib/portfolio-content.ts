...
export async function readContent(): Promise<PortfolioContent> {
  ...
  const response = await fetch(`${process.env.SUPABASE_URL}/rest/v1/portfolio_content?id=eq.main&select=content`, { headers: { apikey: process.env.SUPABASE_SECRET_KEY!, Authorization: `Bearer ${process.env.SUPABASE_SECRET_KEY!}` }, cache: "no-store" });
...
export async function writeContent(content: PortfolioContent) {
  ...
  const response = await fetch(`${process.env.SUPABASE_URL}/rest/v1/portfolio_content`, { method: "POST", headers: { apikey: process.env.SUPABASE_SECRET_KEY!, Authorization: `Bearer ${process.env.SUPABASE_SECRET_KEY!}`, "Content-Type": "application/json", Prefer: "resolution=merge-duplicates" }, body: JSON.stringify({ id: "main", content }) });
